...
bool CMyApp::Init()
{
	...
	ArrayBuffer m_position(std::vector<glm::vec3>{glm::vec3(-20, 0, -20), glm::vec3(-20, 0, 20), glm::vec3(20, 0, -20), glm::vec3(20, 0, 20)});
	ArrayBuffer m_normals(std::vector<glm::vec3>{glm::vec3(0, 1, 0), glm::vec3(0, 1, 0), glm::vec3(0, 1, 0), glm::vec3(0, 1, 0)});
	ArrayBuffer m_texture(std::vector<glm::vec2>{glm::vec2(0, 0), glm::vec2(0, 1), glm::vec2(1, 0), glm::vec2(1, 1)});
	IndexBuffer m_indices(std::vector<uint16_t>{0, 1, 2, 2, 1, 3});

	m_vao.Bind();
	m_vao.AddAttribute(CreateAttribute<  0, glm::vec3, 0, sizeof(glm::vec3)>, m_position);
	m_vao.AddAttribute(CreateAttribute<  1, glm::vec3, 0, sizeof(glm::vec3)>, m_normals);
	m_vao.AddAttribute(CreateAttribute<  2, glm::vec2, 0, sizeof(glm::vec2)>, m_texture);
	m_vao.SetIndices(m_indices);
	m_vao.Unbind();
...
void CMyApp::Render()
{
	...
	m_deferredPointlight.SetUniform("cameraPosition", m_camera.GetEye());
